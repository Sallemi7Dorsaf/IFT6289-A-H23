...
def naive_softmax_loss_and_gradient(
    ...
    Arguments:
    input_vector (for skip-gram) -- numpy ndarray, input word's embedding
                    in shape (word vector length, )
                    (v_i in the pdf handout)
    output_word_idx (for skip-gram) -- integer, the index of the output word
                    (o of u_o in the pdf handout)
    output_vectors (for skip-gram) -- output vectors is
    ...
    grad_input_vec (for skip-gram) -- the gradient with respect to the input word vector
                    in shape (word vector length, )
                    (dL / dv_i in the pdf handout)
    grad_output_vecs (for skip-gram) -- the gradient with respect to all the output word vectors
                    in shape (num words in vocab, word vector length) 
                    (dL / dU)
    
    Hint:
    You can re-use this function for both skip-gram and CBOW model. 
    But the variables' names are for the skip-gram model. 
    Please consdier the corresponding situation for the CBOW model by yourself.
    """
...
def cbow(target_output_word, window_size, context_words, word2_ind,
             input_vectors, output_vectors, dataset,
             word2vec_loss_and_gradient=naive_softmax_loss_and_gradient):
    """ CBOW model in word2vec

    Implement the CBOW model in this function.

    Arguments:
    target_output_word -- a string of the target output word
    window_size -- integer, context window size
    context_words -- list of no more than 2*windowSize strings, the context words
    word2_ind -- a dictionary that maps words to their indices in
              the word vector list
    input_vectors -- input word vectors (as rows) for all words in vocab
                        (V in pdf handout)
    output_vectors -- outside word vectors (as rows) for all words in vocab
                    (U in pdf handout)
    dataset -- dataset for generating negative samples
    word2vec_loss_and_gradient -- the loss and gradient function for
                               ...
                               loss functions you implemented above.
    Return:
    loss -- the loss function value for the CBOW model
            (L in the pdf handout)
    grad_input_vecs -- the gradient with respect to the input word vectors
            (dL / dV, this should have the same shape with V)
    grad_output_vecs -- the gradient with respect to the outside word vectors
                        (dL / dU)
    """
